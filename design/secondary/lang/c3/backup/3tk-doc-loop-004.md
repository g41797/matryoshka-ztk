...
**This is 004, written after the owner's stack-outer ruling.** It carries all  
of `003` and repoints every reference from `3tk-reference-004.md` to  
`3tk-reference-005.md`, which now lives in `matryoshka-3tk/design/`, not in  
this repo's `ref/`. `003` is in `backup/`.

**Nothing else changed.** The invariant, the three modes, the register and the  
rules for moving a module description are `003`'s, word for word.
